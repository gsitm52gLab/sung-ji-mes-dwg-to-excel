...
| 파일 | 역할 |
|---|---|
| `server.py` | FastAPI — 정적 서빙, `/api/chat` SSE, `/api/upload` SSE, 비밀번호 게이트 |
| `context.py` | 대조 CSV → 시스템 프롬프트 (순수 함수) |
| `pipeline.py` | 업로드된 dwg → dxf 변환 → 일람표 대조 |
| `refresh_data.py` | 파이프라인 산출물을 `data/` 스냅샷으로 복사 |
| `data/구역별_대조.csv` | 첫 화면에 쓰는 기본 데이터 스냅샷 |
| `index.html` `static/` | 화면 |

## 도면 업로드

우측 패널에서 **dwg** 를 올리면 서버가 그 자리에서 파이프라인을 돌린다.

```
dwg ──ODAFileConverter──▶ dxf ──deckcheck──▶ 대조 CSV ──▶ 채팅 컨텍스트 교체
```

진행상황 패널은 평소에는 없고 처리 중에만 나타난다. 단계는 SSE 로 실시간
전달된다 (`upload` → `convert` → `compare` → `done`).

리포지토리의 `convert-dwg-to-dxf.sh` 는 변환기를 별도 Docker 컨테이너로 띄우지만
여기서는 그럴 수 없다 — 서버 자체가 컨테이너 안이라 Docker 를 또 띄울 수 없다.
그래서 이미지에 변환기를 같이 넣고 바이너리를 직접 부른다. Qt GUI 앱이라
화면이 없는 서버에서는 `xvfb-run` 으로 가상 디스플레이를 붙인다.

대조 상대가 되는 발주 엑셀은 이미지에 함께 담긴다(`arch-docs/excel`). 즉
"도면 Rev 가 올라왔을 때 기존 발주서와 맞는지" 를 확인하는 흐름이다.

측정치: DETAIL 도면 1.2MB 기준 업로드부터 대조 완료까지 **약 7초**
(amd64 에뮬레이션 기준. Render 는 네이티브라 더 빠르다).

제약:
- **dwg 만 받는다.** 상한 64MB
- 현재 데이터셋은 서버 전체가 하나를 공유한다. 여러 사람이 동시에 올리면
  마지막 것이 이긴다. 시연용으로는 충분하다
- 업로드한 파일은 처리 후 지운다. Render 는 재시작하면 디스크가 날아가므로
  서버가 다시 뜨면 기본 스냅샷으로 돌아간다
- SHOP 도면(32MB → dxf 138MB)은 대조에 필요 없다. 무료 인스턴스 메모리로는
  버거우므로 DETAIL 도면을 올리는 것이 맞다
